...
class Summary_Class_Difference(object):
    def __init__(self, sample_no, df, df_idf, df_T):
        ...
        self.Doc3 = self.df_T['center-context'][sample_no]

        self.file = self.Doc1+self.Doc2
        self.linePerTF = self.set_TF(self.file)
        self.QueryDifference = self.setQueryDifference(self.Doc1, self.Doc2)
        #self.setReferenceCenter()
        self.setReferenceTheme()
        self.setLeft()
        self.setRight()

    def getRightBias(self, length=3):
        R1Query = self.setQueryDifference(self.Doc1, self.Doc2)
        R1 = self.Difference(JM=1, lambda_=0.2, set=0, Query=R1Query)

        R2Query = self.setQueryDifference(self.Doc1, self.Doc3)
        R2 = self.Difference(JM=1, lambda_=0.2, set=0, Query=R2Query)

        # Combine = pd.merge(R1, R2, how='outer')
        R1 = R1[['line', 'TF_Dictionary', 'Line_Length']]
        R2 = R2[['line', 'TF_Dictionary', 'Line_Length']]
        ...
        R2 = R2.head(25)
        '''
        Combine = pd.concat([R1, R2], axis=0, join='outer')
        Combine = Combine.loc[~Combine.index.duplicated(keep='first')]        '''
        # Combine is created from R1 intersect R2
        Combine = pd.merge(R1, R2, how='inner', on=['line'])
        ...
        for i in range(Combine.shape[0]):
            print(Combine.iloc[i]['line'])
        '''
        # Writing to file
        save_path_JM = './JMDifRightBias'+'/'
        ...
        #for i in range(Combine.shape[0]):
        for i in range(length):
            file1.write(Combine.iloc[i]['line']+"\n")
        file1.close()
        return out
        #print("Right Bias of", self.sample_no, "complete")

    def getLeftBias(self, length=3):
        R1Query = self.setQueryDifference(self.Doc2, self.Doc1)
        R1 = self.Difference(JM=1, lambda_=0.2, set=0, Query=R1Query)

        R2Query = self.setQueryDifference(self.Doc2, self.Doc3)
        R2 = self.Difference(JM=1, lambda_=0.2, set=0, Query=R2Query)
        ...
        file1.close()
        return out
        #print("Left Bias of", self.sample_no, "complete")

    def writeCleanSample(self):
    ...
    def preprocess_round1(self, Doc):
        Doc = Doc.lower()
    ...
    def preprocess_round2(self, Doc):
        return ' '.join(word for word in Doc.split() if len(word) > 1)

    def dataRead_func(self):
        openfile = open('data.json')
        jsondata = json.load(openfile)
        df = pd.DataFrame(jsondata)
        openfile.close()
    ...
    def set_TF(self, file):
        ...
        i = 0
        for line in Lines:
            line = self.preprocess_round1(line)
            line = self.preprocess_round2(line)
            d = {}
    ...
    def set_idf(self):
        ...
        for sample_no in range(self.total_doc):
            Doc = self.df['right-context'][sample_no]+self.df['left-context'][sample_no] + \
                self.df['center-context'][sample_no]+self.df['theme-description'][sample_no] + \
                self.df['right-head'][sample_no]+self.df['left-head'][sample_no] + \
                self.df['center-head'][sample_no] + \
                self.df['theme'][sample_no]

            Doc = self.preprocess_round1(Doc)
            Doc = self.preprocess_round2(Doc)
            Docs.append(Doc)
    ...
    def setQueryDifference(self, Doc1, Doc2):
        Doc1 = self.preprocess_round1(Doc1)
        Doc1 = self.preprocess_round2(Doc1)

        Doc2 = self.preprocess_round1(Doc2)
        Doc2 = self.preprocess_round2(Doc2)
        ...
        query_dif_dic = {}
        self.query_dif_len = 0
        for w in dic_Doc1:
            if w not in self.sw:
                if w not in dic_Doc2:
                    query_dif_dic[w] = dic_Doc1[w]
                    self.query_dif_len += query_dif_dic[w]
                elif w in dic_Doc2 and dic_Doc1[w]/dic_Doc2[w] > 2:
                    query_dif_dic[w] = dic_Doc1[w] - dic_Doc2[w]
        return query_dif_dic

    def Difference(self, JM, lambda_, set, Query):
        ...
        for i in range(sentence_count):
            ...
            JM_Score_list[i] = self.JMScore(
                Query=Query, Doc=Line_Dict, lambda_=lambda_, d_len=self.linePerTF.loc[i]['Line_Length'])
        self.linePerTF['JM Score'] = JM_Score_list

        JMDifference = self.linePerTF.sort_values(
    ...
    # Reference: Right Intersection theme -> botom 5 (ground truth1 for Right-Left)
    ...
    # Reference: Left ( can be another ground truth)
    # Right - Left ->   Bias of right (rouge score, compare to left expect low)
    ...
    def setLeft(self):
        ...
        if not os.path.exists(ref_path):
            os.mkdir(ref_path)
        filename = os.path.join(ref_path, "left"+str(self.sample_no)+".txt")
        file1 = open(filename, "w")

        Doc = self.df_T['left-context'][self.sample_no]
        Doc = self.preprocess_round1(Doc)
        Doc = self.preprocess_round2(Doc)
        file1.write(Doc)
        file1.close()
    def setRight(self):
        ...
        if not os.path.exists(ref_path):
            os.mkdir(ref_path)
        filename = os.path.join(ref_path, "right"+str(self.sample_no)+".txt")
        file1 = open(filename, "w")

        Doc = self.df_T['right-context'][self.sample_no]
        Doc = self.preprocess_round1(Doc)
        Doc = self.preprocess_round2(Doc)
        file1.write(Doc)
        file1.close()

    def focusWordFromRight(self):
        word = sorted(self.setQueryDifference(self.Doc1, self.Doc2), reverse=True)
        return word

    def focusWordFromLeft(self):
        word = sorted(self.setQueryDifference(self.Doc2, self.Doc1))
        return word
